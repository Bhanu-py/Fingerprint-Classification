...
saveRDS(tot, 'preprocessed_NIST.rds')
tot <- readRDS('preprocessed_NIST.rds')
...
fig <- plot_ly(Zk %>%
                 as.data.frame %>%
                 mutate(Label = labels %>% as.factor), x = ~PC1, y = ~PC3, z = ~PC5, color = ~labels, colors = c('#636EFA','#EF553B','#00CC96') ) %>%
  add_markers(size = 12)
...
######### lda  ##########################
# sf_lda <- lda(x = X, grouping = labels)
# 
# # cols <- c("n" = "red", "t" = "blue")
# Vlda <- sf_lda$scaling
# Zlda <- X %*% Vlda
# par(mfrow = c(1, 1))
# boxplot(Zlda ~ labels, ylab = expression("Z"[1]), 
...
cv_lasso <- readRDS('models/NIST/cv_lasso_model.rds')
plot(cv_lasso)
...
lasso <- readRDS('models/NIST/lasso_model.rds')
lasso
...
#   type.measure = "class", parallel = TRUE,
#   family = "multinomial", type.multinomial = "grouped")
# saveRDS(cv_ridge, 'models/NIST/cv_ridge_model.rds')
cv_ridge <- readRDS('models/NIST/cv_ridge_model.rds')
plot(cv_ridge)
...
#   family="multinomial", type.multinomial = "grouped")
# saveRDS(ridge, 'models/NIST/ridge_model.rds')
ridge <- readRDS('models/NIST/ridge_model.rds')
