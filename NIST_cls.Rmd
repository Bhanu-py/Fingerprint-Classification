...
knn_search <- tune_bayes(knn_wflow, resamples = tot_folds, initial = 5, iter = 20,
...
```{r}
pred_df <- bind_cols(
  predict(knn_cls_features_fit, dftest),
  predict(knn_cls_features_fit, dftest, type = "prob"))

pred_df$obs <- ytest
# pred_df$.pred <- pmax(pred_df$pred_A, pred_df$pred_L, pred_df$pred_R, pred_df$pred_T, pred_df$pred_W)
colnames(pred_df) <- c("pred","pred_A","pred_L", "pred_R","pred_T","pred_W", "obs")

confusionMatrix(pred_df$pred, ytest)
pred_df %>%
  roc_curve(obs, pred_A:pred_W) %>%
  lines.roc()
```
