...
grid.arrange(pVar2, pVar3, nrow=1, top = grid::textGrob("SFinGe",gp=grid::gpar(fontsize=20,font=3)))
...
abline(v = c(
  quantile(Vk[, 1], 0.05),
  quantile(Vk[, 1], 0.95)), col = "red", lwd = 2,  main='pc1')
...
### Cross validation to choose the lambda value 
...
### Fitting lasso model with the best lambda minimum 
lasso_pca <- glmnet(
              x = Z_train,
              y = Ytrain,
...
dfLassoOpt_pca %>% 
  f_meas(as.factor(pred), as.factor(Ytest))

caret::F_meas(table(as.factor(dfLassoOpt_pca$pred), as.factor(Ytest)))
...
ridge_pca <- glmnet(
  ...
  alpha = 0, parallel=TRUE,    #alpha = 0 forL ridge
  family="multinomial", type.multinomial = "grouped")
...
cm_lasso_full <- confusionMatrix(as.factor(dfLassoOpt$pred), Ytest)
cm_lasso_full
saveRDS(cm_lasso_full, 'results/SFinGe/cm_lasso_full.rds')
dfLassoOpt %>%
  f_meas(as.factor(pred), as.factor(known.truth))
...
cm_ridge_full <- confusionMatrix(as.factor(dfRidgeOpt$pred), as.factor(Ytest))
cm_ridge_full
dfRidgeOpt %>%
  f_meas(as.factor(pred), as.factor(known.truth))
saveRDS(cm_ridge_full, 'results/SFinGe/cm_ridge_full.rds')
